...
- **Video Player**: Integrated streaming player with fullscreen support
...
### Netlify
1. Drag and drop the project folder to Netlify's deployment area, or connect your Git repository
2. Ensure all HTML files are in the root directory
3. The site will be live instantly
4. If you get 404 errors, check that `index.html` exists and redeploy

### Troubleshooting 404 Errors
- Ensure `index.html` is in the root directory of your repository
- For GitHub Pages, the repository must contain `index.html` for the root URL to work
- Commit and push all files to your repository
- Redeploy on Netlify/GitHub Pages after adding files
- Check that file paths in HTML are correct (e.g., `css/style.css`, `js/app.js`)

## Streaming Sources

The player currently shows a placeholder with instructions for integration. To add real streaming:

1. Uncomment the streaming code in `js/app.js` in the `loadPlayer()` function
2. Choose a streaming service that supports TMDB IDs:
   - **VidSrc.to**: `https://vidsrc.to/embed/movie/{id}` or `https://vidsrc.to/embed/tv/{id}`
   - **EmbedSoap**: `https://www.embedsoap.com/embed/movie/{id}`
   - **2Embed**: `https://www.2embed.cc/embed/{id}`
   - Or find another service that provides embed URLs

3. Update the `embedUrl` variable with your chosen service
4. Test the embed URLs directly in a browser first
5. Note: Some services may require HTTPS or have CORS restrictions when testing locally

The placeholder shows the movie/TV ID and type to help with integration testing.
